...
public class RopeProgressBar extends View {
    ...
    protected synchronized void onDraw(final Canvas canvas) {
        ...
        final float slackHeight = 0;// 目前设置为不动,效果为一个横线移动
        final float progressEnd = clamp(lerp(left, end, offset) + (mBounceX * perp(offset)), left, end);
        ...
        final float triangleLeft = clamp(progressEnd - (getTriangleWidth() / 2) - bubbleLeft, 0,
    ...
    public void animateProgress(final int progress) {
        ...
        // Minimum time duration is 500ms because anything faster than that is
        // waaaay too quick
