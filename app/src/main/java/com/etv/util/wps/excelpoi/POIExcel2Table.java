...
public class POIExcel2Table extends BaseExcel2Table<Cell> {
    ...
    protected Cell[][] readExcelCell(Context context, String fileName, int position) throws Exception {
        ...
        if (workbook instanceof HSSFWorkbook) {
            return handlerHSS(context, (HSSFWorkbook) workbook, position);
        } else {
            return handlerXSS(context, (XSSFWorkbook) workbook, position);
        }
    }


    public Cell[][] handlerHSS(Context context, HSSFWorkbook workbook, int position) {
        MyLog.cdl("kermitye");
...
//        int addRow = charts.length * 40;
    ...
    public Cell[][] handlerXSS(Context context, XSSFWorkbook workbook, int position) throws Exception {
        int maxRow, maxColumn;
