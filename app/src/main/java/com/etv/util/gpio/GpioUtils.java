...
public class GpioUtils {
    ...
    */
    ...
    private static boolean writeNode(String path, String arg) {
        MyLog.gpio("Gpio_test set node path: " + path + " arg: " + arg);
        if (path == null || arg == null) {
            MyLog.gpio("set node error");
            return false;
        }
        ...
        try {
            ...
            MyLog.gpio("Gpio_test write node error!! path" + path + " arg: " + arg);
            e.printStackTrace();
            return false;
        } finally {
            try {
    ...
    private static String readNode(String path) {
        ...
        try {
            ...
            MyLog.gpio("IO Exception :" + e.toString());
            e.printStackTrace();
            if (mTime == 0 || SystemClock.uptimeMillis() - mTime < 1000) {
                mFailTimes++;
            }
            if (mFailTimes >= 3) {
                MyLog.gpio("read format node continuous failed three times, exist thread");
            }
        } finally {
            try {
                if (buffer != null) {
