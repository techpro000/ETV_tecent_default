...
public class TraffLisUtil {
    ...
    public static AppTrafficModel trafficMonitor(Context context) {
        ...
        try {
            ...
            PackageInfo packageInfo = manager.getPackageInfo(SharedPerManager.getPackageNameBySp(), 0);
            int uidApp = APKUtil.getUid(context, SharedPerManager.getPackageNameBySp());
            long rxInfo = TrafficStats.getUidRxBytes(uidApp);
            long txInfo = TrafficStats.getUidTxBytes(uidApp);
            appTrafficModel.setAppInfo(packageInfo.applicationInfo);
            appTrafficModel.setDownload(rxInfo);
            appTrafficModel.setUpload(txInfo);
        } catch (Exception e) {
            e.printStackTrace();
        }
